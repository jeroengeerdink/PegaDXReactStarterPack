...
function saveAssignment(caseID, assignmentID, actionID, body) {
  return (dispatch) => {
    dispatch(request(caseID));

    assignmentService.saveAssignment(assignmentID, actionID, body).then(
      (aCase) => {
  ...
  function request(caseID) {
    return { type: actionTypes.ASSIGNMENT_SAVE_REQUEST, caseID };
  }
  function success(aCase) {
    return { type: actionTypes.ASSIGNMENT_SAVE_SUCCESS, aCase };
  }
  function failure(error) {
    return { type: actionTypes.ASSIGNMENT_SAVE_FAILURE, error };
  }
}
